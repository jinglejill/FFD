//
//  ReportSalesByMenuTypeDailyTableViewController.m
//  FFD
//
...
#import "ReportSalesByMenuTypeDailyTableViewController.h"
...
@interface ReportSalesByMenuTypeDailyTableViewController ()
{
...
@implementation ReportSalesByMenuTypeDailyTableViewController
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section
{
    if([collectionView isEqual:colVwData])
    {
        ...
        return ([_salesByItemDataList count]+2)*([_menuTypeList count]+3);
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([collectionView isEqual:colVwData])
    {
        if(btnPercent.selected)
        {
            NSInteger countColumn = [_menuTypeList count] + 3;
            ...
            else if(item/countColumn < [_salesByItemDataList count]+2-1)
            {
                ...
                switch (item%countColumn) {
                    case 0:
                    {
                        cell.lblTextLabel.text = salesByItemData.item2;
                        cell.lblTextLabel.textAlignment = NSTextAlignmentCenter;
                    }
                        break;
                    case 1:
                ...
                if(item%countColumn > 1 && item%countColumn < [_menuTypeList count]+3-1)
                {
                    MenuType *menuType = _menuTypeList[item%countColumn-2];
                ...
                else if(item%countColumn == [_menuTypeList count]+3-1)
                {
                    float sum = [SalesDaily getSumSalesWithSalesDate:salesByItemData.salesDate salesDailyList:_salesDailyList];
        ...
        else
        {
            ...
            else
            {
                ...
                else if(item%countColumn == [_menuTypeList count]+3-1)
                {
                    ...
                    float avg = salesDaily.sales;//[SalesDaily getAvgSalesAllMenuType:_salesDailyList];
                    cell.lblTextLabel.text = [Utility formatDecimal:avg withMinFraction:0 andMaxFraction:0];
                    cell.lblTextLabel.textAlignment = NSTextAlignmentRight;
                }
            }
...
{
    [self loadingOverlayView];
    [self exportImpl:@"ยอดขายแยกตามหมวดรายการอาหารรายวัน"];
}
...
-(void) exportCsv: (NSString*) filename
{
    ...
    else
    {
        ...
        header0 = [NSString stringWithFormat:@"วัน,วันที่%@,ยอดขาย(บาท)\n",header0];
