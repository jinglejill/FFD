//
//  ReportSalesAllMonthlyTableViewController.m
//  FFD
//
...
#import "ReportSalesAllMonthlyTableViewController.h"
...
@interface ReportSalesAllMonthlyTableViewController ()
{
...
@implementation ReportSalesAllMonthlyTableViewController
...
- (void)loadViewProcess
{
    ...
    [self.homeModel downloadItems:dbReportSalesAllMonthly withData:@[startDate,endDate]];
}
        ...
        if([collectionView isEqual:colVwData])
        {
            ...
            else
            {
                ...
                switch (item%countColumn)
                {
                    case 0:
                    {
                        cell.lblTextLabel.text = [Utility dateToString:salesDaily.salesDate toFormat:@"MMM-yy"];
                        cell.lblTextLabel.textAlignment = NSTextAlignmentCenter;
                    }
                    break;
                    case 1:
...
- (void)itemsDownloaded:(NSArray *)items
{
    ...
    if(self.homeModel.propCurrentDB == dbReportSalesAllMonthly)
    {
...
{
    [self loadingOverlayView];
    [self exportImpl:@"ยอดขายรายเดือน"];
}
...
-(void) exportCsv: (NSString*) filename
{
    ...
    else
    {
        ...
        NSString *header0 = @"เดือน,ยอดขาย(บาท)\n";
